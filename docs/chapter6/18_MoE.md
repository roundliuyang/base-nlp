...
在传统的单体神经网络中，如果我们尝试让一个网络同时学习多个截然不同的子任务（例如既学做菜又学修车），往往会出现“**强干扰效应（Strong Interference Effects）**”。这是因为网络的所有权重都参与了所有任务的计算，当网络调整参数以适应任务 A 时，可能会破坏它在任务 B 上已经学到的特征表示。这导致学习速度变慢，泛化能力变差。

为了解决这个问题，论文提出了一种基于 **“分治（Divide and Conquer）”** 策略的系统架构：

1.  **专家网络**：系统包含多个独立的神经网络（可以是简单的前馈网络）。每个专家不再需要处理全局任务，而只需专注于输入空间中的一个局部区域或一类特定的子任务。
2.  **门控网络**：充当协调者的角色。它接收与专家相同的输入 $x$，并输出一组**混合比例（Mixing Proportions）** $p_i$，即选择每个专家的概率。它就像一个软性的随机开关，决定当前的输入案例应该由哪位专家来主导处理。

#### 1.1.2 关键的损失函数设计

该论文最重要的贡献在于设计了特定的机制来鼓励专家之间的**竞争（Competition）**，而非合作。

如果仅仅将所有专家的输出简单线性相加来逼近目标 $\mathbf{y}$，即预测值 $\hat{\mathbf{y}} = \sum p_i \mathbf{E}_i$，并在最终输出上计算误差，专家们会倾向于“合作”。

$$ Loss_{coop} = || \mathbf{y} - \sum_{i} p_i \mathbf{E}_i ||^2 \tag{6.2} $$

在这种合作模式下，为了减小总误差，每个专家都会试图去弥补其他专家的残差。这会导致每个案例都牵动所有专家的权重，依然无法解决“干扰效应”。
...
这一机制实现了**权重的解耦**。误差反向传播时，只有被选中的“胜出者”和门控网络的权重会被更新，其他专家几乎不受影响。这彻底消除了任务间的干扰，实现了“让专业的专家干专业的事”。
...
这意味着 MoE 不再是一个孤立的架构，而成为了一种**可插拔的层**。我们可以在一个深层网络的不同位置（例如第 1 层和第 2 层）分别插入 MoE 模块，每一层都有自己独立的门控网络和专家集合。
...
#### 1.2.2 学习分解的特征表示（Factored Representations）
...
为了让这一构想落地，论文解决了两个关键挑战：

1.  **稀疏门控（Sparsely-Gated）**：
    传统的 Softmax 门控通常会给所有专家分配非零的权重，意味着所有专家都要参与计算。为了解决这个问题，论文引入了 **Noisy Top-k Gating**：
    - 在门控输入中加入可训练的高斯噪声。
    - 仅保留权重最大的 $k$ 个专家（例如 $k=4$），将其余专家的权重强制置为 $-\infty$（即概率为 0）。
...
2.  **负载均衡（Load Balancing）**：
    在稀疏选择下，容易出现“马太效应”。也就是某些专家因为初始化稍好，就被频繁选中，导致它们训练得越来越好，进而被选中的概率更高；而其他专家则因无法获得训练机会而“饿死”。
    为了解决这个问题，作者设计了额外的**辅助损失函数（Auxiliary Loss）**：
    - **Importance Loss**：鼓励所有专家的重要性（即门控权重的累加和）大致相等。
    - **Load Loss**：鼓励所有专家接收到的样本数量大致相等。
    这强制门控网络“雨露均沾”，确保所有专家都能得到充分的训练。
...
#### 2.1.2 Top-2 Gating 与模型分片

GShard 明确了 MoE 层的标准计算范式。对于输入 Token $\mathbf{x}$，MoE 层的最终输出 $\mathbf{y}$ 是由门控网络 $\mathcal{G}$ 选择的专家输出的加权和：
...
每个专家 $\mathbf{E}_i$ 内部通常就是一个标准的双层全连接网络（FFN）：

$$ \mathbf{E}_i(\mathbf{x}) = \mathbf{W}_{out} \cdot \text{ReLU}(\mathbf{W}_{in} \cdot \mathbf{x}) \tag{6.5} $$

在此基础上，GShard 解决了两个核心问题：

1.  **Top-2 Gating（Top-2 门控）**：
    此前的研究通常使用 Top-k（如 k=4）或更复杂的门控。GShard 发现，**每个 Token 只路由给 2 个专家**（Top-2）就足够了。
    -   **第一专家**：选择权重最高的专家，保证主要任务的处理。
    -   **第二专家**：根据权重概率随机选择或直接选择第二高权重的专家，引入辅助处理和一定的随机性，有助于负载均衡。
    这种 Top-2 策略成为了后来（包括 Mistral）的标准配置。

2.  **模型并行（Model Parallelism / Sharding）**：
    单张 GPU/TPU 无法装下 6000 亿参数。GShard 将专家网络**分片（Shard）**分布在不同的设备上。
    ...
    -   **动态路由**：当一个 Token 需要被某个专家处理时，如果该专家不在当前设备上，系统会通过高效的 **All-to-All** 通信原语，将该 Token 发送到目标设备。计算完成后，再将结果传回。
...
为了在硬件（如 TPU）上高效运行，必须处理动态路由带来的负载不均问题。

1.  **专家容量（Expert Capacity）**：
    由于硬件通常要求静态的 Tensor 形状，模型必须预设每个专家能处理的最大 Token 数量：
...
2.  **可微负载均衡损失（Differentiable Load Balancing Loss）**：
    为了尽量减少 Token 丢弃，Switch Transformer 引入了一个辅助损失函数，鼓励 Token 均匀分布到所有专家：
    ...
    *   $P_i$ 是**预期**路由给专家 $i$ 的概率总和（门控网络觉得专家 $i$ 应该接收多少人）。
    ...
    这个公式的直观含义是：我们希望“实际去的”和“计划去的”向量的点积最小。只有当两者都均匀分布（即所有专家的负载都相等）时，这个 Loss 才会达到最小值。这迫使 Router 不偏科，雨露均沾。
...
2.  **数据质量优先**：
    GLaM 的研究团队发现，高质量的数据对于大模型的性能至关重要。他们开发了一套高质量的文本质量分类器，对原始网页数据进行了严格的过滤。实验表明，使用过滤后的高质量数据训练的模型，在各项任务上的表现均优于使用未过滤海量数据的模型。
...
*   **架构参数**：它拥有约 **470 亿（47B）** 的总参数量（Sparse Parameters），但对于每个 Token，仅激活 **130 亿（13B）** 参数（Active Parameters）。这使得它在推理时拥有 13B 模型的速度，却能发挥出 47B 模型的容量。
...
我们统计了不同领域数据（如 arXiv, PubMed, Wikipedia 等）在不同层（Layer 0, 15, 31）的专家分配比例。如图 6-12 可以看出，同一行（即同一个专家）在不同列（不同数据集）上的颜色深浅非常接近。这意味着，**无论输入文本属于哪个领域，Router 选择各专家的概率分布几乎是一样的**。专家似乎更多地是根据**语法（Syntax）**和**Token 结构**（如缩进、介词）来分工，而非人类定义的知识领域。
...
与 Mistral 采用的“粗粒度”专家不同，DeepSeekMoE 引入了两个关键策略，如图 6-13 所示：
...
DeepSeek-R1 不仅在常规任务上表现出色，更通过**大规模强化学习（RL）** 具备了强大的逻辑推理能力。
